...
if __name__ == "__main__":
    ...
    for run_idx in range(__N_RUNS):  # For each CV-fold
        ...
        # Define the DNN
        model = get_sino_dnn(input_shape=np.shape(train_data)[1:],
                             lr=0.001)
    ...
    plt.ylim([45, 102.5])
    plt.title('DNN Performance on Training (Gen-2) and '
              'Validation (Gen-1) Sets',
              fontsize=26)
    plt.tight_layout()
    plt.show()
    plt.savefig(os.path.join(__FIG_OUT_DIR,
                             'dnn_train_g2_validate_g1_aucs.png'),
                dpi=600, transparent=True)
    plt.savefig(
        os.path.join(__FIG_OUT_DIR,
                     'dnn_train_g2_validate_g1_aucs_not_transparent.png'),
