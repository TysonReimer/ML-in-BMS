...
if __name__ == "__main__":
    ...
    for adi_id in unique_adi_ids:  # For each adipose ID
        ...
        aucs_here = np.zeros([__N_RUNS, ])
        accs_here = np.zeros([__N_RUNS, ])
        sens_here = np.zeros([__N_RUNS, ])
        spec_here = np.zeros([__N_RUNS, ])
        accs_opt_thresh_here = np.zeros([__N_RUNS, ])
        sens_opt_thresh_here = np.zeros([__N_RUNS, ])
        spec_opt_thresh_here = np.zeros([__N_RUNS, ])
        ...
        for run_idx in range(__N_RUNS):
            ...
            # Store results in arrays as percents
            aucs_here[run_idx] = 100 * auc
            accs_here[run_idx] = 100 * acc
            sens_here[run_idx] = 100 * sens
            spec_here[run_idx] = 100 * spec
            accs_opt_thresh_here[run_idx] = 100 * test_acc_opt
            sens_opt_thresh_here[run_idx] = 100 * test_sens_opt
            spec_opt_thresh_here[run_idx] = 100 * test_spec_opt
        ...
        adi_results[adi_id] = [aucs_here, accs_here, sens_here, spec_here,
                               accs_opt_thresh_here, sens_opt_thresh_here,
                               spec_opt_thresh_here]
    ...
    for adi_id in unique_adi_ids:
        ...
        report_metrics(adi_results[adi_id][0], adi_results[adi_id][1],
                       adi_results[adi_id][2], adi_results[adi_id][3],
                       logger)
